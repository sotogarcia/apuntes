# Formación por competencias

- Estrategia educativa que se orienta a equipar a los estudiantes con las competencias
- Se entran en el puesto de trabajo y no tanto en el proceso de aprendizaje.
- Poner el saber en acción

## Competencia:

- conjuntos de habilidades, conocimientos, actitudes y valores—necesarias para afrontar los desafíos específicos de su entorno profesional y personal.

### Tipos

- **Competencia profesional**   : aptitud misma labor en diferentes contextos
- **Competencia laboral**      : conocimientos, habilidades, destrezas y actitudes para un puesto determinado.

# Programación de acciones formativas

- Secuenciación organizada y planificada de los contenidos asociados a un módulo

### Factores básicos

- Qué, cuándo y cómo enseñar
- Qué, cuándo y cómo evaluar

### Elementos integrales

- **Qué enseñar**   : objetivos y contenidos
- **Cómo enseñar**  : metodología y actividades
- **Cuánto enseñar**: temporización
- Qué, cómo y cuándo evaluar: evaluación

### Carácterísticas

- Contextualizada
- Útil
- Funcional
- Realista
- Flexible y dinámica
- Abierta y plural
- Sistemática

## Título, justificación y destinatarios

### Título

- **Certificados o programa formativo** : basta citar el nombre
- **Acciones diseñadas a medida**      : requiere descripción básica

### Justificacion

- Motivos de impartición
- Qué necesidades satisface
- Normativa aplicable

### Destinatarios

- Carácterísticas
- Necesidades de cualificación
- Perfil profesional
- Perfil formativo


# Objetivos

- Enunciados que describen las conductas que el alumnado tiene que ser capaz de llevar a cabo al finalizar su formación.

## Carácterísticas

- Significativos y relevantes
- Precisos
- Conocidos por el alumnado
- Alcanzables y realistas
- Observables y medibles

## Clasificación

- **Nivel de concreción**:
    - Generales     : se formulan de forma ámplia
    - Específicos   : situación concreta  alcanzable a corto plazo. Deriban de los generales.
        - Conceptuales      (saber qué)
        - Procedimentales   (saber hacer)
        - Actitudinales     (saber ser/estar)
    - Operativos    : observables y medibles
- **Nivel en el que expresan la conducta**:
    - Cognitivos    : conocimientos, destrezas o habilidades carácter intelectual
    - Afectivos     : actitudes y hábitos
    - Psicomotores  : destrezas o habilidades motoras, manipulación, manejo de máquinas,...

## Formulación

 - concreta y precissa
 - infinitivo + objeto o contenido + finalidad

> Objetivos
>   - "Competencia general" es el objetivo general para la actividad formativa
>   - Cada módulo tiene sus objetivos específicos, que son los C (C1, C2,...)

# Contenidos

## Tipos

- **Conceptuales o teóricos**: se corresponden con el saber o saber qué
- **Procedimentales o prácticos**: elevado componente de practicidad
- **Actitudinales o profesionalizadores**: valores normas y actitudes


## Selección

- **Objetivos**
- **Destinatarios**
- **Modalidad de impartición**
    - On-line
    - Presencial

## Secuenciación

### Recomendaciones

- Integrar los relacionados entre sí
- De más conocidos a menos conocidos
- Partir de ideas previas del alumnado
- De más fácil a más difícil

### Tipología

- Conceptuales
- Prácticos
- Profesionalizadores

### Modalidad

- Presencial
- On-line

## Normas de redacción y estilo

...

# Metodología

- Conjunto de procedimientos didácticos a seguir para consecución de los objetivos propuestos

## Métodos didácticos

- Organización racional de procedimientos y técnicas de enseñanza, con el objetivo...

- **Según la forma de razonamiento**
    - **Deductivo**: se presentan conceptos, principios, teorías... se extraen conclusiones
    - **Inductivo**: se parte de casos concretos para alcanzar generalidades
- **Grado de participación del alumnado**
    - Pasivos o afirmativos
        - Expositivo
        - Demostrativo
    - Activos por elaboración
        - Interrogativo
        - Por descubrimiento
        - Tutorial

## Técnicas

- **Lección magistral**: Exposición teórica por parte del docente para transmitir conocimientos a un amplio grupo de estudiantes.
- **Simulación**: Réplica de situaciones reales en un entorno controlado para practicar habilidades o tomar decisiones.
- **Resolución de problemas**: Método que implica identificar, analizar y encontrar soluciones a situaciones complejas de forma práctica.
- **Estudio de casos**: Análisis detallado de situaciones reales o hipotéticas para desarrollar habilidades de pensamiento crítico y toma de decisiones.
- **Proyecto**: Tarea o conjunto de tareas con objetivos específicos que los estudiantes deben completar, aplicando conocimientos y habilidades en un contexto práctico.

## Modalidad de impartición

- **Presencial**
- **Online**

# Actividades

- Prácticas o ejercicios diseñados y planificados con la finalidad de ayudar al alumnado a lograr los objetivos propuestos.

## Estructura

- Módulo o unidad a la cual está asociada
- Número de actividad dentro del módulo
- Duración o tiempo máximo
- Denominación
- Enunciado
- Recursos y medios
- Pautas docentes
- Dimensiones competenciales
    conocimientos
    destrezas
    actitudes
- Criterios de evaluación que vas a emplear

> En FPE puedes emplear el objetivo específico para denominar cada actividad
> EN certificados **debes** utilizar la capacidad (C) o el criterio de evaluación (CE)


## Tipos

- Iniciación
- Conocimientos previos
- Desarrollo y compresión
- Aplicación
- Refuerzo
- Repetición
- Relación o comprensión
- Reflexión
- Memorización

## Dinámicas de grupo

- Presentación
- Mural o cartel
- Debate dirigido
- Philips 6/6
    - Grupos de 6
    - Discuten 6 minutos para llegar a postura común
    - Puesta en común de estas opiniones
- Estudio de caso
- Lluvia de ideas
- Inversión de roles
- Juego de roles
- Simulación

## Criterios de elaboración y redacción

- *Elaboración*
    - Cálculo realista del tiempo
    - Indivisual o en grupo
    - Papel del docente
    - Coherencia con objetivos
    - Perfil del destinatario
    - Mayor protagonismo al alumnado
- *Redacción*:
    - Verbo de acción en tercera persona y en tiempo futuro
    - Detalles e instrucciones
    - Orden de realización (secuenciación de la actividad)

# Recursos didácticos

- Material que se utiliza con una finalidad didáctica en el contexto de una acción formativa

## Tipos

- Instalaciones
- Organización del espacio y tiempo
    - Infraestructura
    - Normativa
    - Carácterísticas del alumnado
- Materiales y tecnológicos
    - Fungibles
    - No fungible
    - Tecnológico
- Documentales
    - Bibliografía
        - Esencial
        - Complementaria
    - Anexos
- Humanos

# Procedimiento evaluador

- Proceso sistemático de identificación, recogida y tratamiento de datos

## Tipos

- **Según el momento de evaluación**
    - Inicial o predictiva
    - Contínua
    - Final
- **Agente evaluador**
    - Autoevaluación    : alumno a sí mismo
    - Heteroevaluación  : por parte del docente
    - Coevaluación      : alumnos a otros alumnos

## Instrumentos de evaluación

- Son los medios que permiten regoger y registrar la información extraida del proceso de evaluación

### Carácterísticas:

- **Fiabilidad**: resultados similares en diferentes momentos para el mismo alumnado
- **Validez**: grado de precisión con que la prueba mide lo que pretende medir
- **Objetividad**: independencia del juicio del docente
- **Adecuación**: relación con los contenidos estudiados
- **Practicidad**: eficaces en cuanto al tiempo empleado en su elaboración

## Ejemplos

- **Pruebas de conocimiento**
    - Escritas
        - Objetivas (tipo test)
        - Subjetivas (ensayo o desarrollo)
    - Orales
- **Pruebas de competencias procedimentales**
    - Pruebas prácticas
    - Listas de cotejo: lista de conductas observables (Sí|No)
    - Hojas de evaluación práctica: cuadros de doble entrada (Puntuables 1-10)
- **Pruebas de actitudes**
    - Observación directa
    - Entrevistas personales.

## Ponderaciones

- Ponderar es determinar el porcentaje o peso de cada módulo o unidad tendrá sobre la calificación global.

Asignar el **porcentaje sobre la nota final** para cada módulo

> Ejemplo
> Módulo 1: su superación conlleva el 25% de la nota final
> Módulo 2: su superación conlleva el 50% de la nota final
> Módulo 3: su superación conlleva el 25% de la nota final


Puedes asignar **pesos a ciertas competencias individuales**

> Ejemplo
> Evaluación del módulo 1
> Adquisición de contenidos conceptuales (conocimientos): 50%
> Adquisición de contenidos conceptuales (destrezas): 25%
> Adquisición de contenidos conceptuales (actitudes): 25%

**Legislación**: Oren ES1897/2013

- Apto (suficiente): de 5 a 6,9
- Apto (notable): de 7 a 8,9
- Apto (sobresaliente): de 9 a 10

## Revisión

- Seguimiento formativo: ???

## Objetivos

- Detectar posibles fallos o desajustes en el proceso de enseñanza y aprendizaje
- Corregirlos en el momento en que se estén produciendo
- Identificar los procesos que ocurren de forma adecuada para mantenerlos y mejorarlos

## Importante

- Proceso integral: progreso del alumno / propia programación didáctica
- Actualización constante

---

# Temporización diaria

## Esquema

- Objetivos: Específicos del módulo
- Contenidos
    - Conceptuales
    - Procedimentales
    - Actitudinales
    - Recursos
- Metodología
- Actividades